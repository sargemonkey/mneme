...
- **`Mneme.Benchmarks.LoCoMo`** — accuracy benchmark vs Mem0/Zep. **Full apples-to-apples (Exp 11): 90.4% on the Mem0-comparable scope — statistical parity with Mem0's 92.5%** (within ~2pp on n=385), with every controllable variable matched: gpt-4o answerer+judge, top-k 200, Mem0 answer procedure + lenient J-score judge, LoCoMo cat 1–4 (adversarial excluded, as Mem0 does), ~6,297 tokens/query vs their ~6,956. Per-category: temporal 95.6%, multi-hop 91.9%, single 88.5%, open-domain 81.0%. **Efficiency variant (Exp 10): 83.6% with gpt-4o-mini at ~715 tokens/query (~9× less context).** Almost the entire apparent gap to Mem0 was measurement + configuration (scope, judge, answer prompt, model, retrieval depth), not memory-layer capability. Levers: two-pass distill (statements + separate triples), KG subject-triple answer-context supplement, hybrid semantic+BM25, date-stamped context. See `ANALYSIS.md` for all 11 experiments incl. the attribution diagnosis, KG validation, and the alignment ladder.
