...
- **`Mneme.Benchmarks.LoCoMo`** — accuracy benchmark vs Mem0/Zep. **Apples-to-apples headline (Exp 10): 83.6% on the Mem0-comparable scope** (LoCoMo cat 1–4; Mem0 excludes adversarial), with Mem0-aligned answer procedure + lenient J-score judge, gpt-4o-mini answerer, ~715 tokens/query vs Mem0's ~6,956 (≈10× less context). Per-category: single 88.5%, temporal 81.1%, multi-hop 78.4%, open-domain 66.7%; adversarial 25% (out of Mem0 scope). Levers: two-pass distill (statements + separate triples), KG subject-triple answer-context supplement, hybrid semantic+BM25, date-stamped context. The remaining gap to Mem0's 92.5 is answerer-model strength + retrieved-context size, both independent of the memory layer. See `ANALYSIS.md` for all 10 experiments incl. the attribution diagnosis + KG validation.
