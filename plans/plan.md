...
- **`Mneme.Benchmarks.LoCoMo`** — accuracy benchmark vs Mem0/Zep, full LoCoMo-10 (1,986 Q). **Full apples-to-apples parity (Exp 12): 89.6% Mem0-comparable (gpt-4o, top-200) — within ~3pp of Mem0's reported 92.5%** on the full 1,540-question in-scope set, at ~6,254 tokens/query vs their ~6,956; per-category single 91.4, temporal 90.3, multi-hop 88.3, open-domain 75.0. **Efficient variant: 80.3% (gpt-4o-mini, top-25, ~724 tokens/query — ~9× less context).** Every controllable variable matched to Mem0's runner (model, retrieval depth, answer procedure, lenient J-score judge, cat 1–4 scope). Almost the entire apparent gap to Mem0 was measurement + configuration, not memory-layer capability. Adversarial (out of Mem0 scope) shows a real recall-depth-vs-attribution trade-off: 29.1% @ k25 → 17.5% @ k200. Publishable summary in `benchmarks/Mneme.Benchmarks.LoCoMo/RESULTS.md`; all 12 experiments in `ANALYSIS.md`.
