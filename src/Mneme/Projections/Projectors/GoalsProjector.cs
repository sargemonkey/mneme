...
/// <summary>Projects <see cref="GoalPayload"/> events into <c>projection_goals</c>.</summary>
public sealed class GoalsProjector : IProjector
{
    /// <inheritdoc/>
    ...
    {
        if (e.Payload is not GoalPayload p) return;
        using var cmd = c.CreateCommand();
        cmd.Transaction = tx;
        cmd.CommandText = """
            INSERT INTO projection_goals(workstream_id, event_id, statement, state,
    ...
    {
        using (var del = c.CreateCommand()) { del.Transaction = tx; del.CommandText = "DELETE FROM projection_goals;"; del.ExecuteNonQuery(); }
        var n = 0;
